...
<div class="mt-10 grid md:grid-span-3 gap-6">
        ...
        <div class="bg-white p-6 rounded shadow-md">
            ...
            <select name="ticket_quantity[]" class="mt-2 w-full border rounded p-1">
                @for($i = 0; $i <= 5; $i++)
            ...
            <div class="flex justify-between items-center mt-4">
                ...
                <a href="{{ route('checkout.form', ['ticketType' => 'vip', 'jumlah' => 1]) }}" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition">
                    Beli Tiket VIP
                </a>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded shadow-md">
            ...
            <input type="hidden" name="ticket_type[]" value="festival_a">
            <select name="ticket_quantity[]" class="mt-2 w-full border rounded p-1">
                @for($i = 0; $i <= 5; $i++)
                <option value="{{ $i }}">{{ $i }}</option>
                @endfor
            ...
            <div class="flex justify-between items-center mt-4">
                <p class="text-gray-500 text-xs">Tersisa 100 tiket</p>
                <a href="{{ route('checkout.form', ['ticketType' => 'festival', 'jumlah' => 1]) }}" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition">
